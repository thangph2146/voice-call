...
export default function useWebRTCAudioSession(
  ...
  function getOrCreateEphemeralUserId(): string {
    ...
    if (!ephemeralId) {
      ...
      logger.debug(`Created ephemeral user message: ${ephemeralId}`, null, 'WebRTC');
    }
    return ephemeralId;
  }
  ...
  function setupAudioVisualization(stream: MediaStream) {
    try {
      ...
      logger.error('Failed to setup audio visualization', error, 'WebRTC');
    }
  }
  ...
  function getSelectedVoice(): SpeechSynthesisVoice | null {
    ...
    if (voice) {
      ...
      if (selectedVoice) {
        logger.debug('Using selected voice', { voice: selectedVoice.name, lang: selectedVoice.lang }, 'WebRTC');
    ...
    if (vietnameseVoice) {
      logger.debug('Using fallback voice', { voice: vietnameseVoice.name, lang: vietnameseVoice.lang }, 'WebRTC');
    }
  ...
  async function initializeSpeech() {
    try {
      logger.info('Initializing speech recognition and synthesis for Vietnamese', null, 'WebRTC');
      ...
      if (!SpeechRecognition) {
        logger.error('Speech Recognition not supported in this browser', null, 'WebRTC');
      ...
      logger.info('Speech recognition initialized for Vietnamese', { lang: 'vi-VN' }, 'WebRTC');
      ...
      logger.error('Failed to initialize speech for Vietnamese', error, 'WebRTC');
      throw error;
    }
  }
    ...
    try {
      ...
      if (synthRef.current) {
        ...
        if (selectedVoice) {
          ...
          utterance.pitch = selectedVoice.name.toLowerCase().includes('female') ? 1.1 : 1.0; // Higher pitch for female voices

          logger.debug('Using selected voice for speech', {
            voice: selectedVoice.name,
            lang: selectedVoice.lang,
          ...
          logger.warn('No voice selected, using default settings', null, 'WebRTC');
        }

        utterance.onstart = () => {
          setStatus(`Đang nói với giọng ${selectedVoice?.name || 'mặc định'}...`);
        ...
        utterance.onerror = (event) => {
          logger.error('Speech synthesis error', event, 'WebRTC');
      ...
      logger.error('Error processing speech with Gemini', error, 'WebRTC');
      setStatus(`Lỗi xử lý: ${error}`);

      // Fallback response
      const fallbackText = "Xin lỗi, tôi gặp sự cố khi xử lý yêu cầu của bạn. Vui lòng thử lại.";
  ...
  async function startSession() {
    try {
      ...
      logger.error('startSession error', err, 'WebRTC');
      setStatus(`Lỗi: ${err}`);
      stopSession();
    }
  }
  ...
  async function sendTextMessage(text: string) {
    ...
    try {
      ...
      logger.info('Gemini text response received', { response: aiText }, 'WebRTC');
      ...
      if (synthRef.current) {
        ...
        if (selectedVoice) {
          ...
          logger.debug('Using selected voice for text message', {
            voice: selectedVoice.name,
            lang: selectedVoice.lang
          }, 'WebRTC');
        } else {
          utterance.lang = 'vi-VN';
          utterance.pitch = 1.1;
          logger.warn('No voice selected for text message, using default', null, 'WebRTC');
        }
        ...
        utterance.onerror = (event) => {
          logger.error('Text message speech synthesis error', event, 'WebRTC');
      ...
      logger.error('Error sending text message with Gemini', error, 'WebRTC');
      setStatus(`Lỗi xử lý tin nhắn: ${error}`);

      // Fallback response
      const fallbackText = "Xin lỗi, tôi gặp sự cố khi xử lý tin nhắn của bạn. Vui lòng thử lại.";
      const fallbackMessage: Conversation = {
        id: uuidv4(),
        role: "assistant",
        text: fallbackText,
